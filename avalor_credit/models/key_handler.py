...
class KeyHandler:
    ...
    def __read_pkcs12_private_key(self):
        ...
        bytes_pkcs12 = self.key_pair_file
        #files = file_content.split("\r\n")


        # with open(self.key_pair_file.decode("utf-8"), "rb") as filepointer:
       ...
       #     filepointer.close()
        ...
        # raise UserError(_(loaded_pri_key))

        return loaded_pri_key

    def __read_certificate(self):
        loaded_pub_key = None
        #with open(self.cert_file.decode("utf-8"), "r") as file_pointer:
            #lines = file_pointer.readlines()
            #max_lines = len(lines)
        serialized_certificate = self.cert_file.decode("utf-8")
