...
def authenticate_token_for_user(token):
    ...
    raise werkzeug.exceptions.HTTPException(
        response=error_response(*(401, "Authentication", "Your token could not be authenticated."))
    )
...
def get_data_from_auth_header(header):
    ...
    """
    # raise werkzeug.exceptions.HTTPException(
    #     response=error_response(500, "Invalid header", str(header))
    # )
    # normalized_token = header.replace("Basic ", "").replace("\\n", "").encode("utf-8")
    normalized_token = header
    try:
        decoded_token_parts = (
            # base64.b64decode(normalized_token).decode("utf-8").split(":")
            normalized_token.split(":")
        )
    except TypeError:

        raise werkzeug.exceptions.HTTPException(
            response=error_response(
                401, "Authentication", "Your token could not be authenticated."
            )
        )
    ...
    if len(decoded_token_parts) == 1:
        ...
        raise werkzeug.exceptions.HTTPException(
            response=error_response(401, "Authentication", "Do you need token")
        )
...
def route(*args, **kwargs):
    ...
    def decorator(controller_method):
        ...
        def controller_method_wrapper(*iargs, **ikwargs):
            ...
            db_name, user_token = get_data_from_auth_header(api_key)
            db_name = "avalor"
...
def wrap__resource__create_one(modelname, context, data, success_code, out_fields):
    ...
    try:
        ...
        return error_response(400, type(e).__name__, str(modelname))
