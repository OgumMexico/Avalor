...
def successful_response(status, data=None):
    ...
    try:
         ...
         mesage = {
             "Message": "Dato Guardado Con exito",
             "Id":data.ids
         }
...
def route(*args, **kwargs):
    ...
    """
    headers = {}
    headers["x-api-key"] = request.httprequest.headers.get("x-api-key")
...
def wrap__resource__create_one(modelname, context, data, success_code, out_fields):
    ...
    """
    # jsondat = str(data)
    # data=json.dumps(jsondat)
    # data['id_stp'] = int(data['id_stp'])
